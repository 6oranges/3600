...
void Scene::CalculateRightAndUp()
{
	...
	double theta=fov/2*pi/180;
	double u=atan(theta);
	double r=screen_x*u/screen_y;
...
void Scene::CreateRay(int x, int y, Vector3 &ray)
{
	...
	Point3 film = ww_bl+(ww_br-ww_bl)*right_ratio + (ww_tl-ww_bl)*up_ratio; // Switch top bottom from video
	// Generate a ray from the eye to the film position.
	ray = film-eye; // Big problem in video
	ray.Normalize();
}
						...
						)
{
	...
	if (determinant<0){ // ray doesn't collide sphere
		return false;
	}
	...
	if (h<0){
		return false; // Sphere behind us
	}
	else if (l<0){ // Inside sphere
		distance = h;
		intersection_point = eye+ray*distance;
		intersection_normal = s.origin-intersection_point;
...
void Scene::CastRay(const Vector3 &ray,float &r,float &g,float &b)
{
	...
	// double r,g,b; // This line is wrong from video
	r = g = b = 0;
	if(closest_hit_sphere != -1)
	{
		...
		for (int l=0;l<num_point_lights;l++){
			Vector3 vector_to_light = pl[l]-closest_point;
			...
			if(angle_reduction>0){
				...
				Vector3 sumvec = vector_to_light + vector_to_light;
				sumvec.Normalize();
				double temp = DotProduct(sumvec,closest_normal);
				if (temp>0){
					double e = s[closest_hit_sphere].specular_exponent;
					double specular_portion = pow(temp,e);
					r+= specular_portion*s[closest_hit_sphere].specular[0];
					g+= specular_portion*s[closest_hit_sphere].specular[1];
					b+= specular_portion*s[closest_hit_sphere].specular[2];
				}
			}
		}
	}
